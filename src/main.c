...
void get_wasd_input(int *sz, int *sx, State *s, int m) {
	if (!g->typing) {
...
void get_jump_input(float *vy, float *dy) {
	if (!g->typing) {
...
int estimate_steps(float vx, float vy, float vz, float speed, float dy, double dt) {
...
float handle_gravity(float dy, float ut) {
	if (g->flying) {
		dy = 0;
	}
	else {
...
void handle_movement(double dt) {
	...
	handle_ortho();
	handle_fov();
	get_wasd_input(&sz, &sx, s, m);

	float vx, vy, vz;
	get_motion_vector(g->flying, sz, sx, s->rx, s->ry, &vx, &vy, &vz);

	get_jump_input(&vy, &dy);

	float speed = g->flying ? 20 : 5;
	int estimate = estimate_steps(vx, vy, vz, speed, dy, dt);
